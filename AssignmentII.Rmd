...
beta_gradient_desc = function(tuning, Y,X, beta, alpha){
  ...
  gradient_lasso = lamba1 * t(matrix(1, nrow=ncol(X), ncol=1))%*%sign(beta)
...
# Run the function with given tuning parameters
tuning <- c(0.1, 0.1)
beta_gradient_desc(tuning, Y, X, beta_true, alpha =0.1)

result <- regopt(tuning, Y, X, nitermax = 1000)
