...
```{r}
loo_cv = function(Y,X, ridge_regression=TRUE){
  # Setup up cross-validation grid matrix
  cv_grid = matrix(data=NA, nrow=nrow(X), ncol = ncol(X)+1) 
  # Setup cross validation error grid matrix
  cv_error_grid = matrix(data=NA, nrow=nrow(X), ncol = ncol(X)+1)
  lambda_range = seq(0,5, length.out=100)
  for( i in 1:nrow(X)){
    # Leave one observation out
    x_training_set = X[-i,]
    x_validation_set = X[i,]
    # Use the first column as coefficient column
    cv_grid[i,1] = x_validation_set
    # Use first column as coefficient column
    cv_error_grid[i, 1] = x_validation_set
    for (j in 1: length(lambda_range )){
      ...
      if(ridge_regression){
        tuning = c(lambda, 0)
        result = regopt(tuning, Y, X, nitermax = 20)
      }else{
        tuning = c(0, lambda)
        result = regopt(tuning, Y, X, nitermax = 20)
      }
       ...
       y_hat = X%*%result$beta
       cv_grid[i,j] = y_hat
       cv_error_grid[i, j] = (y_validation - y_hat)^2
    }
  }
  colnames(cv_grid) = c("x", paste0("lambda", lambda_range))
  mse = apply(cv_error_grid[,-1], 2, mean)
  return (list(fitted_values = cv_grid, errors = cv_error_grid, mse=mse))
}
loo_cv(Y_train,X_train)
```