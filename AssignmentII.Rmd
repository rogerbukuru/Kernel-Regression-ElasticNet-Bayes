...
regopt = function(tuning, Y, X, nitermax=20){
  ...
  p = ncol(X)
  beta  = rnorm(p, 0, 0.01) # Ensures random starting point
...
lambdas_small <- seq(0, 1, length.out = 100)  # Finer grid for smaller lambdas
lambdas_large <- seq(1.1, 5, length.out = 100) # Coarser grid for larger lambdas
lambdas <- c(lambdas_small, lambdas_large)
